...
const DisplayExpensesReport: React.FC<TypeProps> = ({ getUser, getBudget }) => {
  return (
    <div className="flex flex-col w-full px-0 md:px-28 py-10 space-y-10">
      ...
      {getBudget && (
        <div className="flex flex-col w-full h-full max-h-full space-y-1 px-3 md:px-0 overflow-y-scroll scrollbar-thin scrollbar-thumb-zinc-500 scrollbar-track-zinc-300">
          ...
          {getBudget.map((budget: { id: string, name: string, counter: string, currency: string, expense: number, budgetDetails: any }) => {
            ...
            return (
              <div key={budget.counter} className="flex flex-col md:flex-row items-start md:items-center w-full cursor-default rounded-md divide-y md:divide-y-0 divide-x-0 md:divide-x divide-zinc-400 border border-zinc-400">
                <div className="flex items-center justify-start px-5 py-2 w-full max-w-full md:max-w-xl font-bold text-lg text-zinc-700">
                  {budget.name}
                </div>
                <div className="flex items-center justify-start px-5 py-2 w-full max-w-full md:max-w-[10rem] font-bold text-lg text-zinc-700">
                  <span className="font-light text-sm">Expenses:</span>&nbsp;{getExpenses.length}
                </div>
                <div className="flex items-center justify-start px-5 py-2 w-full max-w-full md:max-w-full text-zinc-700">
                ...
                <div className="flex items-center justify-center px-5 py-2 w-full max-w-full md:max-w-[10rem] text-zinc-700">
